...
* @version $Id: MultipleChoice10Richtext.php 2 2013-12-09 16:39:31Z weinert $
*/
...
*/
require_once(dirname(__FILE__).'/MultipleChoice10.php');
...
class        PapayaQuestionnaireQuestionMultipleChoice10Richtext
  extends    PapayaQuestionnaireQuestionMultipleChoice10
  ...
  */
  public function getQuestionXML($questionId, $selectedAnswer = FALSE, $replacements = array()) {
    ...
    if ($connector = $this->getConnector()) {
      ...
      $result .= sprintf(
        '<explanatory-text>%s</explanatory-text>'.LF,
        base_object::getXHTMLString(
          $this->replaceTemplateValues(
            $this->getConfigurationOption('explanatory_text'),
            $replacements
          )
        )
      );
